...
app.factory('PdfService', function($q, $location, $translate, $timeout, CommonService, Const, StorageService) {
	...
	var print = function($scope) {
		...
		// iOS の場合はポップアップブロック回避のため、通信前にあらかじめ新規タブを開く
		var win = null;
		if (isIOS) {
			win = window.open('about:blank', '_blank');
			if (win != null) {
				win.document.write('<html><head><title>印刷</title></head><body style="text-align:center;padding-top:50px;font-family:sans-serif;"><p>PDFを作成しています。しばらくお待ちください...</p></body></html>');
			}
		}
		...
		CommonService.httpModal($scope, Action + "/print", param, false,
			function(resolveData) {
				// 成功処理
				if (!CommonService.isError(resolveData)) {
					var printUrl = CommonService.getPrintFile(resolveData.fileName);
					var relativeUrl = printUrl.replace(/^https?:\/\/[^\/]+/, '');

					if (isIOS) {
						// 【iOSの場合】あらかじめ開いておいたタブで PDF を読み込む
						if (win != null && !win.closed) {
							win.location.href = relativeUrl;
						} else {
							window.location.href = relativeUrl;
						}
					} else {
						// 【PCの場合】Blob 経由で iframe 印刷ダイアログを自動起動
